...
function fnFormatNumber(value: any, fallback = '0', decimal: number = 2) {
  return fnValidationNumber(
    value,
    Intl.NumberFormat('en-US').format(
      parseFloat(value).toFixed(decimal) as any
    ),
    fallback
  )
}

function fnFormatNumberOne(value: any, fallback = '0', decimal: number = 1) {
  return fnValidationNumber(
    value,
    Intl.NumberFormat('en-US').format(
      parseFloat(value).toFixed(decimal) as any
...
export function fnFormatPercent(value: any, fallback = '0%', decimal: number = 2) {
  return fnValidationNumber(
    value,
    (parseFloat(value) * 100).toFixed(decimal) + '%',
    fallback
  )
}

function fnFormatPercentOne(value: any, fallback = '0%', decimal: number = 1) {
  return fnValidationNumber(
    value,
    (parseFloat(value) * 100).toFixed(decimal) + '%',
    fallback
...
function fnFormatOneHundred(value: any, fallback = '0%', decimal: number = 2) {
  return fnValidationNumber(
    value,
    (parseFloat(value) * 100).toFixed(decimal) as any,
    fallback
  )
}

function fnFormatOneHundredOne(
  value: any,
  fallback = '0%',
  decimal: number = 1
) {
...
function fnFormatOneHundredZero(
  value: any,
  fallback = '0%',
  decimal: number = 0
) {
